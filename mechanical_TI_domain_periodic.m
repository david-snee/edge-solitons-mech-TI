...
function mechanical_TI_domain_periodic(Nr,Ns)
str = ['NETW_periodic_rectangle' int2str(Nr) 'x' int2str(Ns)];
...
%% PERIODIC BONDS
TN=sum([N1 N2 N3 N4 N5 N6]);

function adm = p_bond_y0y2(y0L,y2R)
    ry0 = rsy0(y0L,1);
    sy0 = rsy0(y0L,2);
    ry2 = rsy2(y2R,1);
    sy2 = rsy2(y2R,2);
    x0L=0; x1L=0; y2L=0; y1L=0; x2L=0; x0R=0; x1R=0; y0R=0; y1R=0; x2R=0;
    adm = (ry0-ry2==0) && (sy0-sy2==-(Ns-1));
end

for y0L = 1:N2
    for y2R = 1:N6
        if p_bond_y0y2(y0L,y2R)
            bonds = [bonds;x0L,y0L,x1L,y1L,x2L,y2L,x0R,y0R,x1R,y1R,x2R,y2R];
        end
    end    
end

function adm = p_bond_y2y0(y2L,y0R)
    ry2 = rsy2(y2L,1);
    sy2 = rsy2(y2L,2);
    ry0 = rsy0(y0R,1);
    sy0 = rsy0(y0R,2);
    x0L=0; x1L=0; y0L=0; y1L=0; x2L=0; x0R=0; x1R=0; y2R=0; y1R=0; x2R=0;
    adm = (ry2-ry0==0) && (sy2-sy0==Ns-1);
end

for y2L = 1:N6
    for y0R = 1:N2
        if p_bond_y2y0(y2L,y0R)
            bonds = [bonds;x0L,y0L,x1L,y1L,x2L,y2L,x0R,y0R,x1R,y1R,x2R,y2R];
        end
    end    
end

function adm = p_bond_x0x2(x0L,x2R)
    rx0 = rsx0(x0L,1);
    sx0 = rsx0(x0L,2);
    rx2 = rsx2(x2R,1);
    sx2 = rsx2(x2R,2);
    y0L=0; x1L=0; y2L=0; y1L=0; x2L=0; x0R=0; x1R=0; y0R=0; y1R=0; y2R=0;
    adm = (rx0-rx2==0) && (sx0-sx2==-(Ns-1));
end

for x0L = 1:N1
    for x2R = 1:N5
        if p_bond_x0x2(x0L,x2R)
            bonds = [bonds;x0L,y0L,x1L,y1L,x2L,y2L,x0R,y0R,x1R,y1R,x2R,y2R];
        end
    end    
end

function adm = p_bond_x2x0(x2L,x0R)
    rx2 = rsx2(x2L,1);
    sx2 = rsx2(x2L,2);
    rx0 = rsx0(x0R,1);
    sx0 = rsx0(x0R,2);
    x0L=0; x1L=0; y0L=0; y1L=0; y2L=0; y0R=0; x1R=0; y2R=0; y1R=0; x2R=0;
    adm = (rx2-rx0==0) && (sx2-sx0==Ns-1);
end

for x2L = 1:N5
    for x0R = 1:N1
        if p_bond_x2x0(x2L,x0R)
            bonds = [bonds;x0L,y0L,x1L,y1L,x2L,y2L,x0R,y0R,x1R,y1R,x2R,y2R];
        end
    end    
end
