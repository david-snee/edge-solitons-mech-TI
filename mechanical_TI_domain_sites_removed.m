...
function mechanical_TI_domain_sites_removed(Nr,Ns,Q)
str = ['NETW_sites_removed' int2str(Nr) 'x' int2str(Ns)];
...
for s = 0:Ns-1
    for r = 0:Nr-1
        if admit_x0y0(r,s)            
                if find(ismember(find(Q1==r),find(Q2==s))==1)~=0
                    SR0= [SR0;r s];
                else
                    rsx0 = [rsx0;r,s];
                    rsy0 = [rsy0;r,s];
